...
class __PickerUiState extends State<_PickerUi> {
  bool _isLink = false;
  int _maxLevel = -1;
  late List<List<PickerItem>?> _dataList;
  late List<FixedExtentScrollController?> _ctrlList;
  late List<int?> _selects;
  ...
  void initState() {
    ...
    _dataList = List.filled(_maxLevel, null, growable: false);
    _ctrlList = List.filled(_maxLevel, null, growable: false);
    _selects = List.filled(_maxLevel, null, growable: false);
    _initData();
  }
  ...
  void _getMaxLevel(List? data, int level) {
    ...
    if (_isLink) {
      //列表联动
      for (PickerItem item in data as Iterable<PickerItem<dynamic>>) {
  ...
  void _initData() {
    for (int i = 0; i < _maxLevel; i++) {
      ...
      if (_isLink) {
        if (i == 0) {
          _dataList[i] = widget.data as List<PickerItem<dynamic>>?;
        } else {
